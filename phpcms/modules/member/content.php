...
class content extends foreground {
	...
	public function published() {
        ...
        /*这是资金申报的控制器部分*/
         function publishedCapital(){
         
            //资金申报的数据表是ax_form_finance：
            $tableName='ax_form_zjsb';
            
            //自己写的数据库连接
            $con = mysql_connect("localhost","root","");
            if(!$con) die('数据库连接错误！');
            
            mysql_select_db('shangwuting',$con) or die('数据库选取失败！');  
            
            mysql_set_charset("utf8", $con);//设定编码
            
            $sql="select * from $tableName where userid =".param::get_cookie('_userid');
            ...
            include template('member', 'content_published_capital'); 
        }
        
         /*这是我要参展的控制器部分*/
         function publishedJoin(){
         
            //我要参展的数据表是ax_form_finance：
            $tableName='ax_form_join_show';
            ...
            include template('member', 'content_published_join');
